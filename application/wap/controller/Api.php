...
/*	public function fengkong($price,$pro)
	...
	public function free_order_all(){
		...
		$where = array('uid'=>$user_id,'is_timing'=>1,'ostaus'=>0);

		if($type == 2){
			$where['ostyle'] = 0;// " and ostyle = 0";
		}elseif ($type == 3){
			$where['ostyle'] = 1;
		}
	...
	public function getkdata($pid=null,$num=null,$interval=null,$isres=null)
	{
		...
		$all_data['topdata'] = array(
										...
										'highest'=>$pro['High'],
										'close'=>$pro['Close'],
										'zhangfu'=> sprintf('%.2f',($pro['Price']-$pro['Open'])/$pro['Open']*100),
										'protitle'=>$pro['ptitle'],
		...
		return base64_encode(json_encode($all_data));
