...
function toggle_order_confirm_panel(type) {
  ...
  if(type == 'lookup'){
    var typename = '{$Think.lang.iin_mz}';
    order_type = 0;
    
    $('.order_type').removeClass('fall');
    $('.order_type').addClass('rise');
  }else{
    var typename = '{$Think.lang.iin_md}';
    order_type = 1;
...
function addorder(){
    ...
    if(order_price > my_money){
        err_info('{$Think.lang.jin_zjbzqxcz}');
        return;
    }
    if(order_price < order_min_price){
        err_info('{$Think.lang.jin_zxxzjew}'+order_min_price);
        return;
    }
    if(order_price > order_max_price){
        err_info('{$Think.lang.le_zdxzjew}'+order_max_price);
        return;
    }
    ...
    $.post(posturl,postdata,function(resdata){
        ...
        if(resdata.type == 1){
            ...
            if(resdata.data.ostyle == 0){
                $('.pay_order_type').html('{$Think.lang.iin_mz}');
                $('.pay_order_type').addClass('rise');
                $('.pay_order_type').removeClass('fall');
            }else{
                $('.pay_order_type').html('{$Think.lang.iin_md}');
    ...
    $.each(resorderlist,function(k,v){
        ...
        if(_end_time >0){
            ...
            if(v.ostyle == 0){
                ...
                var ostyle_name = "{$Think.lang.iin_mz}";
                if(chaprice >0){
                ...
                var ostyle_name = "{$Think.lang.iin_md}";
