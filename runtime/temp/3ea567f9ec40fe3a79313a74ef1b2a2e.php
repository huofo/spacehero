<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:78:"/www/wwwroot/agc.bxtrade.vip/application/admin/view/order/orderlist_trade.html";i:1637939671;s:61:"/www/wwwroot/agc.bxtrade.vip/application/admin/view/head.html";i:1637561064;s:61:"/www/wwwroot/agc.bxtrade.vip/application/admin/view/menu.html";i:1631099310;s:61:"/www/wwwroot/agc.bxtrade.vip/application/admin/view/foot.html";i:1553756524;}*/ ?>
<!DOCTYPE html>
...
<section id="main-content">
    <section class="wrapper">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 mar-10">
                            <div class="input-group">
                                ...
                                <input type="text"  name="orderid"  class="form-control" value="<?php echo !empty($getdata['oid'])?$getdata['oid']:''; ?>" placeholder="输入订单号/外部订单号"/>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-3 mar-10">
                            <div class="input-group">
                                ...
                                <select name="status" id="" class="selectpicker show-tick form-control">
                                    <option value="">默认不选</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 1): ?> selected="selected" <?php endif; ?>  value="1">待支付</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 2): ?> selected="selected" <?php endif; ?>  value="2">已支付待确认</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 3): ?> selected="selected" <?php endif; ?>  value="3">已完成</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 4): ?> selected="selected" <?php endif; ?>  value="4">申诉中</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 5): ?> selected="selected" <?php endif; ?>  value="5">未支付失效</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 6): ?> selected="selected" <?php endif; ?>  value="6">买家取消订单</option>
                                    <option <?php if(isset($getdata['status']) && $getdata['status'] == 7): ?> selected="selected" <?php endif; ?>  value="7">卖家拒绝订单</option>
                                </select>
                            </div>
                        </div>
                    </div>
        ...
        <a href="<?php echo url('order/orderlist_trade'); ?>"><button type="submit" class="btn btn-danger">搜索全部</button></a>&nbsp;&nbsp;&nbsp;&nbsp; <span class="color_red">&nbsp;&nbsp;<strong>默认为7天内订单</strong></span>
        <br><br>
        <div class="row">
            <div class="col-lg-12">
                <section class="panel">
                    ...
                    <table class="table table-striped table-advance table-hover">
                        ...
                        <tr>
                            ...
                            <td><?php echo $vo['order_sn']; ?><br><a href="/admin/Order/orderlist_shevles.html?orderid=<?php echo $vo['old_order_sn']; ?>"><?php echo $vo['old_order_sn']; ?></a>
                            </td>



                            <td><a><?php echo !empty($vo['nickname'])?$vo['nickname']:$vo['user_id']; ?></a><br><?php echo !empty($vo['ip'])?$vo['ip']:''; ?></td>

                            <?php if($vo['trade_type'] == 0): ?>
                            <td><span  class="color_red">购买</span><br><?php echo date("Y-m-d H:i:s",$vo['addtime']); ?></td>
                            <?php elseif($vo['trade_type'] == 1): ?>
                            <td><span  class="color_green">出售</span><br><?php echo date("Y-m-d H:i:s",$vo['addtime']); ?></td>
                            <?php endif; ?>

                            <td><a><?php echo $vo['old_username']; ?></a></td>
                            <td><?php echo $vo['currency_type']; ?><br><?php echo $vo['symbol']; ?></td>

                            <td><?php echo $vo['t_num']; ?><br><?php echo $vo['one_price']; ?></td>
                            ...
                            <td><?php if($vo['pay_done_img']): ?>
                                <a href="/public/uploads/<?php echo $vo['pay_done_img']; ?>" target="_blank" >查看</a>
                            ...
                            <td>
                                ...
                                <?php if($vo['status'] == 2 && $vo['trade_type'] == 0 && $vo['api_order_sn'] != ''): if($vo['notify_state'] == 0): ?>
                                     等待
                                     <?php elseif($vo['notify_state'] == 1): ?>
                                     成功
                                     <?php elseif($vo['notify_state'] == 2): ?>
                                      失败
                                       <?php endif; endif; ?>
                            ...
                            <td><?php echo !empty($vo['api_order_sn'])?$vo['api_order_sn']:''; ?><br><?php echo !empty($vo['sdorder_sn'])?$vo['sdorder_sn']:''; ?></td>


                            <td>
                                <?php if($vo['status'] == 1): if($vo['trade_type'] == 0): ?>

                                <button class="btn btn-primary btn-xs" onclick="jujue_order(<?php echo $vo['id']; ?>)">拒绝</button>&nbsp;&nbsp;

                                <?php endif; endif; if($vo['status'] == 1  || $vo['status'] == 6): if($vo['trade_type'] == 0): ?>
                                     <button class="btn btn-primary btn-xs" onclick="pass_order(<?php echo $vo['id']; ?>)">通过</button>
                                     <?php endif; endif; if($vo['status'] == 2  &&  $vo['trade_type'] == 0  &&  $vo['api_order_sn'] != ''): ?>
                                
                                <button class="btn btn-primary btn-xs" onclick="bufa_order(<?php echo $vo['id']; ?>)">通知</button>
...
<script>
    ...
    function pass_order(id) {
        layer.open({
            ...
            yes: function(index){
                //do something
                var formurl = "<?php echo Url('order/pass_torder'); ?>";
                var data = 'id='+id;
    ...
    function jujue_order(id) {
        layer.open({
            ...
            yes: function(index){
                //do something
                var formurl = "<?php echo Url('order/jujue_torder'); ?>";
                var data = 'id='+id;
