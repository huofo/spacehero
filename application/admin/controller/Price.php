...
class Price extends Base
{
	...
	public function pricelist()
	{
		...
		foreach ($list as $key => $v) {
            ...
            //手动入金
            $all_cash_mapshoudong['uid'] = $v['uid'];
            $all_cash_mapshoudong['bptype'] = 2;
            $all_cash_mapshoudong['isverified'] = 1;
            if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
                $all_cash_mapshoudong['btime'] = array('between time',array($getdata['starttime'],$getdata['endtime']));
            }
            $_list[$key]['all_cash_shoudong'] = $balance->where($all_cash_mapshoudong)->sum('bpprice');
            ...
            //净入金
            $_list[$key]['res_cash'] = $_list[$key]['all_res'] +( $_list[$key]['all_cash_shoudong']) - $_list[$key]['all_cash'];
			...
			$lc_map['title'] = '留仓费';
			if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
				$lc_map['time'] = array('between time',array($getdata['starttime'],$getdata['endtime']));
			}
			$_list[$key]['all_lc'] = $price_log->where($lc_map)->sum('account');



            //总盈亏
			$ls_ploss_map['uid'] = $v['uid'];
			if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
				$ls_ploss_map['selltime']  = array('between time',array($getdata['starttime'],$getdata['endtime']));
			}
			$_list[$key]['ls_ploss'] = $order->where($ls_ploss_map)->sum('ploss');
			

			
			//总手续费
			$sx_ploss_map['uid'] = $v['uid'];
			if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
				$sx_ploss_map['selltime']  = array('between time',array($getdata['starttime'],$getdata['endtime']));
			}
			$_list[$key]['all_sx_fee'] = $order->where($sx_ploss_map)->sum('sx_fee');
            $_list[$key]['all_sx_fee'] = $_list[$key]['all_sx_fee']*(-1);
	...
	public function myprice()
	{
		...
		$list = db('price_log')->alias('pl')->field('pl.*,u.username,u.utel')
				->join('__USERINFO__ u','u.uid=pl.uid')
				->where($map)->order('id desc')->paginate($pagenum,false,['query'=> $getdata]);;
		$all = db('price_log')->alias('pl')->field('pl.*,u.username,u.utel')
				->join('__USERINFO__ u','u.uid=pl.uid')
				->where($map)->sum('account');
			//var_dump($all);	
		$this->assign('list',$list);
		$this->assign('getdata',$getdata);
		return $this->fetch();
	...
	public function pricesta()
	{
		...
		//线下入金总额
		$all_xia_map['bptype'] = 2;
		$all_xia_map['isverified'] = 1;
		if(!empty($uids)){
            $all_xia_map['uid'] = array('IN',$uids);
        }
		if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
			$all_xia_map['btime'] = array('between time',array($getdata['starttime'],$getdata['endtime']));
		}
		$_list['all_xia_map'] = $balance->where($all_xia_map)->sum('bpprice');
		...
		$his_ploss_map = array();
		if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
			$his_ploss_map['selltime'] = array('between time',array($getdata['starttime'],$getdata['endtime']));
		}
		if(!empty($uids)){
